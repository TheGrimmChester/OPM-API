...
- Feature: **open PR only after review PASS** — coding complete chains `run-review` (not deliver). Auto-deliver (`OPM_IMPL_AUTO_DELIVER`, default on) runs after validated review PASS and a complete plan, then autopilot merges/`done` or the human gate stops at `human_review`. `run-pipeline` already used this order; the chained implement path now matches. Shared `deliverTaskPR` keeps pipeline and post-review delivery aligned.

- Feature: **auto pipeline from backlog to done** — `POST …/tasks` defaults to autopilot (`humanReviewRequired=false`) and always enqueues `run-pipeline`. The pipeline auto-approves require-review-before-coding, drains every coding subtask, opens a PR only after review PASS (all subtasks complete), then moves to `done` (merging when a PR exists). Pass `humanReviewRequired: true` to stop at `human_review` instead.
...
- Feature: **per-task human review gate** — task field `humanReviewRequired` (default **true**). When true, after coding the bot chains automated review (+ qa-fix if needed), opens a PR only after review PASS when auto-deliver is on, and stops at `human_review`; no auto-merge or auto-complete. When false (autopilot), the bot runs review → deliver on PASS → qa-fix loop as needed → merges the delivery PR via ORA `scm:pr` (when available) → moves the task to `done`. The task session workspace (`$OPM_JOB_TMP/tasks/{projectId}/{specId}/repo`) is retained through implementation and review until deliver / human gate / done.

- Feature: **task-scoped implementation session** — all coding subtasks for a spec share one read-write workspace under `$OPM_JOB_TMP/tasks/{projectId}/{specId}/repo`. Each subtask still runs in a new ephemeral runner container, but file changes persist across steps; the change set merges incrementally. Successful steps auto-chain to the next pending coding subtask (`OPM_IMPL_AUTO_CHAIN`, default on). When coding completes, the bot chains `run-review`; auto-deliver after review PASS is optional (`OPM_IMPL_AUTO_DELIVER`, default on). Pause retains the workspace.
