...
OPM-API depends on shared libraries that live in their own repositories:
...
repository builds from a checkout of OPM-API alone.
...
A library change reaches OPM-API only once it is pushed and the version here is
bumped:
...
Use a tag instead of `@main` where one exists: `open-logger-go`; `open-tenant-go` are tagged, the rest resolve to commit pseudo-versions.
