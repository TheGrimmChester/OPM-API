...
func TestAuthMiddlewareProjectACL(t *testing.T) {
	...
	deny := httptest.NewRequest(http.MethodGet, "/api/projects", nil)
	...
	allow := httptest.NewRequest(http.MethodGet, "/api/projects", nil)
	...
	adminTok, err := openauth.MintUserJWT(authGate.Secret, "admin", "admin", "opm-api", 0)
	if err != nil {
		t.Fatal(err)
	}
	adminReq := httptest.NewRequest(http.MethodGet, "/api/projects", nil)
